...
dictionary = {
    ...
    "epsilon_psi": 1e-5, # coefficient for viscous relaxation of free surface where interface velocity F = (1 - visc_coeff*curvature)
    ...
    # output options
    "outfile": './Results/relax_' + str(h) + 'x_' + str(dt).split('.')[-1] + 't',
    ...
    "true_dx": False,
    "true_dy": False,
    "true_ls1": "(0.05*np.cos(2*2*3.14159265*X)*(np.exp(-ti/(2*2*2*3.14159265*800/2000/9.81)/((0.5*2*2*3.14159265 + np.sinh(0.5*2*2*3.14159265)*np.cosh(0.5*2*2*3.14159265))/np.sinh(0.5*2*2*3.14159265)**2))) + (Y-0.5))",
    "true_t": False,
    "true_ls2": False}
