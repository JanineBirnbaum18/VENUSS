...
ndigits = int(floor(log10(ins.tf/ins.dt))) + len(str(ins.dt).split('.')[-1].lstrip('0')) + len(str(ins.dt).split('.')[0].rstrip('0'))
...
if ins.vtk:
    ...
    mesh.export_to_vtk(ins.outfile + '/' + ins.outfile + '_mesh.vtk')

# Levelset definition:
# free surface
if ins.free_surface:
    ls1 = gf.LevelSet(mesh, 1, ins.ls1p, ins.ls1s)
...
if ins.temp & ins.solidification:
    if not ins.free_surface:
        mls = gf.MeshLevelSet(mesh)
    ls2 = gf.LevelSet(mesh, 1, ins.ls2p, ins.ls2s)
    mls.add(ls2)
...
if ins.topography:
    if not (ins.free_surface | (ins.temp & ins.solidification)):
        mls = gf.MeshLevelSet(mesh)
    ls3 = gf.LevelSet(mesh, 1, ins.ls3p, ins.ls3s)
    mls.add(ls3)
...
if ins.plots:
    ...
    fig.savefig(ins.outfile + '/' + ins.outfile + '_mesh.pdf')
...
if ins.free_surface | (ins.temp & ins.solidification) | ins.topography:
    ...
    mim_all = gf.MeshIm(mesh)
    mim = gf.MeshIm(mesh)
    mim_integ = gf.MeshIm(mesh)
...
rho = ones_p * ins.rho1
if ins.free_surface:
    rho[ls1_p > 0] = ins.rho2
if ins.topography:
    rho[ls3_p < 0] = ins.rho3
...
# Compressibility
beta = ones_p * ins.beta1
if ins.free_surface:
    rho[ls1_p > 0] = ins.beta2
if ins.topography:
...
if ins.temp:
    ...
    if ins.free_surface:
        kappa[ls1_t > 0] = ins.kappa2
    if ins.topography:
        kappa[ls3_t < 0] = ins.kappa3
...
if ins.temp:
    ...
    eta[T_p <= (ins.vftc - 273)] = ins.max_eta

    if ins.free_surface:
        eta[ls1_p > 0] = ins.eta2
    if ins.topography:
        eta[ls3_p < 0] = ins.eta3
else:
    eta = eval(ins.eta_exp.replace('exp', 'np.exp').replace('T', str(ins.T0))) * ones_p
    if ins.free_surface:
        eta[ls1_p > 0] = ins.eta2
    if ins.topography:
        eta[ls3_p < 0] = ins.eta3
...
if (type(ins.f_x) is type(None)) and (type(ins.f_y) is type(None)):
    ...
    if (type(ins.f_x) is float) or (type(ins.f_x) is int):
        ...
        f_xi = eval(ins.f_x.replace('y', 'y_p').replace('x', 'x_p'))
    ...
    if (type(ins.f_y) is float) or (type(ins.f_y) is int):
        ...
        f_yi = eval(ins.f_y.replace('y', 'y_p').replace('x', 'x_p'))
        ...
        for i, bound in enumerate(bounds):
            ...
            # Neumann boundaries
                ...
                if (type(eval('ins.' + bound + '_flux_T')) is float) or (type(eval('ins.' + bound + '_flux_T')) is int):
                    data_t = -eval('ins.' + bound + '_flux_T') * ones_t * kappa
                if type(eval('ins.' + bound + '_flux_T')) is str:
                    data_t = -eval(eval('ins.' + bound + '_flux_T')) * kappa
                md.add_initialized_fem_data(bound + 'data_t', mft, [data_t, data_t])
...
if not ins.restart:
    ...
    Previous_d = d_init
    D = ones_u * 0
    D[eval(ind_u)] = Previous_d[eval(ind_u)] + ins.dt*md.variable('u')
...
if ins.restart:
    ...
    if ins.free_surface:
        hf.create_dataset('last_ls',data=Ls1)
    if ins.temp & ins.solidification:
        hf.create_dataset('last_ls',data=(T-Tg)/Tg)
    if ins.topography:
        hf.create_dataset('last_ls',data=ls3.values(0))
...
for i, ti in enumerate(np.arange(tstart, ins.tf, ins.dt)):
    ...
    if ins.temp:
        ...
        eta = eval(ins.eta_exp.replace('exp', 'np.exp').replace('T', 'T_p').replace('vft','ins.vft'))
    ...
    D[eval(ind_u)] = - BDF2 / BDF0 * Previous_d[eval(ind_u)] - BDF1 / BDF0 * Previous2_d[
        eval(ind_u)] + ins.dt / BDF0 * md.variable('u')
    D_sol[i + 1, :] = D[ind_sol]
