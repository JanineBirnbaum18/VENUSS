...
for outfilei in outfiles:
    ...
    if ins.free_surface:
        md.add_nonlinear_term(mim_all,"psis*Test_psis + psik*Grad_psis.Grad_Test_psis - psi*Test_psis")
    ...
    if ins.temp:
        ...
        if 'SUPG' in ins.stab_t:
            ...
            md.add_nonlinear_term(mim, tau_GLS_t + '*' + time_int_t + '*(u.Grad_Test_t)')
    ...
    if not ins.restart:
        ...
        if ins.vtk:
            ...
            if ins.free_surface:
                ...
                if ins.true_ls1:
                    ...
                    err_ls1[0] = np.sqrt(np.sum((eval('-'+ins.true_ls1.replace('X','pts[0,:]').replace('Y','0')) - pts[1, :]) ** 2)) / pts.shape[1]
        ...
        if ins.free_surface:
            ...
            relax_speed = ins.epsilon_psi * (curvature-mean_curvature)/eta_grid
    ...
    for i, ti in enumerate(np.arange(tstart, ins.tf+ins.dt, ins.dt)):
        ...
        if ins.free_surface:
            ...
            relax_speed = ins.epsilon_psi * (curvature - mean_curvature)
